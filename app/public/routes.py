...
def fill_in_task_info(task):
    if type(task) == models.EventUser:
        ...
        if event.recurrence:
            task.link = url_for('public.recurring_event_view', cid=event.calendar.id, id=event.recurrence.id)
        else:
            task.link = url_for('public.scheduled_event_view', cid=event.calendar.id,  id=event.id)
        ...
        task.link = url_for('public.edit_profile')
    else:
        raise
...
def calendar_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if not calendar.permissions.check_editor(current_user):
            ...
            calendar.save()
            flash('Changes Saved', 'success')
            return redirect(url_for('public.calendar_view', id=id))
...
def scheduled_event_publish(cid, id):
    ...
    return redirect(url_for('public.event_info', cid=calendar.id, id=event.id))
...
def scheduled_event_delete(cid, id):
    ...
    return redirect(url_for('public.event_list'))
...
def recurring_event_publish(cid, id):
    ...
    event.is_draft = False
    event.save()
    return redirect(url_for('public.recurring_event_info', cid=calendar.id, id=event.id))
...
@public.route('/calendar/<cid>/event-recurring/<id>/delete')
