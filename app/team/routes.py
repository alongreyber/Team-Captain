...
@team.route('/home')
def index():
    return redirect(url_for('team.user_profile'))
...
def assignment_complete(id):
    ...
    if not au.completed:
        ...
        au.save()
    return redirect(url_for('team.assignment_view', id=assignment.id))
...
def assignment_info(id):
    ...
    if not assignment.permissions.check_editor(current_user):
        abort(404)
    if assignment.is_draft:
        return redirect(url_for('team.assignment_edit', id=id))
...
def fill_in_task_info(task):
    if type(task) == models.EventUser:
        ...
        task.link = url_for('team.event_view', id=event.id)
        ...
        task.link = url_for('team.assignment_view', id=assignment.id)
    else:
        raise
...
def send_notification(notification_settings, ou):
    ...
    for time in notification_settings.notification_dates:
        ...
        notification.link = url_for('team.task_redirect', id=ou.id)
...
def calendar_home():
    ...
    for e in events:
        ...
        if e.calendar.permissions.check_editor(current_user):
            if e.recurrence:
                e_new['url']    = url_for('team.recurring_event_info', cid=e.calendar.id, id=e.recurrence.id)
            else:
                e_new['url']    = url_for('team.event_info', cid=e.calendar.id, id=e.id)
        else:
            if e.recurrence:
                e_new['url']    = url_for('team.recurring_event_view', cid=e.calendar.id, id=e.recurrence.id)
            else:
                e_new['url']    = url_for('team.scheduled_event_view', cid=e.calendar.id, id=e.id)
...
def calendar_new():
    ...
    return redirect(url_for('team.calendar_edit', id=calendar.id))
...
def calendar_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if not calendar.permissions.check_editor(current_user):
            ...
            calendar.save()
            flash('Changes Saved', 'success')
            return redirect(url_for('team.calendar_view', id=id))
...
def scheduled_event_new():
    ...
    if not calendar:
        ...
        return redirect(url_for('team.calendar_home'))
    everyone_role = models.Role.objects(name='everyone').first()
    ...
    event.save()
    return redirect(url_for('team.scheduled_event_edit', id=event.id))
...
def scheduled_event_edit(id):
    ...
    if not event.is_draft:
        return redirect(url_for('team.scheduled_event_view', cid=calendar.id, id=event.id))
...
def scheduled_event_publish(cid, id):
    ...
    return redirect(url_for('team.event_info', cid=calendar.id, id=event.id))
...
def event_info(cid, id):
    ...
    if event.is_draft:
        if event.recurrence:
            return redirect(url_for('team.recurring_event_edit', cid=calendar.id, id=event.recurrence.id))
        else:
            return redirect(url_for('team.scheduled_event_edit', cid=calendar.id, id=event.id))
...
def event_user_edit(cid, id):
    ...
    if form.validate_on_submit():
        ...
        if eu.sign_in > eu.sign_out:
            ...
            eu.save()
            flash('Changes Saved', 'success')
            return redirect(url_for('team.event_info', id=event.id, cid=calendar.id))
...
def recurring_event_new():
    ...
    return redirect(url_for('team.recurring_event_edit', id=event.id))
...
def recurring_event_edit(id):
    ...
    if not event:
        abort(404)
    if not event.is_draft:
        return redirect(url_for('team.recurring_event_view', cid=calendar.id,  id=event.id))
    if not event.calendar.permissions.check_editor(current_user):
        abort(404)
    all_calendars = models.Calendar.objects
    allowed_edit_calendars = []
...
def recurring_event_publish(cid, id):
    ...
    event.is_draft = False
    event.save()
    return redirect(url_for('team.recurring_event_info', cid=calendar.id, id=event.id))
...
def recurring_event_view(cid, id):
    ...
    if not event:
        abort(404)
    if event.is_draft:
        return redirect(url_for('team.recurring_event_edit', cid=calendar.id, id=event.id))
...
def recurring_event_delete(cid, id):
    ...
    if not calendar.permissions.check_editor(current_user):
        abort(404)
    recurring_event = models.RecurringEvent.objects(calendar=cid, id=id).first()
    if not recurring_event:
        abort(404)
    if not recurring_event.is_draft:
        events = models.Event.objects(recurrence=recurring_event)
...
def topic_new():
    ...
    topic.save()
    return redirect(url_for('team.topic_edit', id=topic.id))
...
def topic_delete(id):
    ...
    return redirect(url_for('team.wiki_home'))
...
def article_new():
    ...
    article.topic = topic
    article.save()
    return redirect(url_for('team.article_edit', id=article.id, tid=topic.id))
...
def article_delete(tid, id):
    ...
    return redirect(url_for('team.topic_view', id=tid))
...
def assignment_new():
    ...
    assignment.save()
    return redirect(url_for('team.assignment_edit', id=assignment.id))
...
def assignment_edit(id):
    ...
    if not assignment.is_draft:
        return redirect(url_for('team.assignment_view', id=id))
...
def assignment_publish(id):
    ...
    if not assignment:
        abort(404)
    if not assignment.is_draft:
        redirect(url_for('team.assignment_edit', id=id))
    ...
    assignment.save()
    return redirect(url_for('team.assignment_info',id=id))
...
def assignment_duplicate(id):
    ...
    new_assignment.save()
    return redirect(url_for('team.assignment_edit', id=new_assignment.id))
...
def assignment_delete(id):
    ...
    if not assignment.is_draft:
        for au in assignment.users:
            ...
            au.delete()
    assignment.delete()
    return redirect(url_for('team.assignment_list'))
